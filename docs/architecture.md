...
| **7. 产品化** | 伏笔追踪 + 真相文件 + 导出 + UX | 伏笔/支线追踪表、长篇真相文件、导出(MD/EPUB)、整书阅读/就地编辑、进度/字数看板 | 🚧 伏笔追踪 + 真相文件 + UX + 导出子项已完成 |
...
- **伏笔/支线追踪**：项目工作台可维护伏笔、支线、资源线、情感线及其状态；追踪项同步 MemoryChunk，并注入写作/审校/修订上下文；成稿章节可生成状态建议，由用户手动应用；总览页提供跨章追踪仪表盘。
...
- **章节就地编辑**：章节正文可在项目工作台手写/编辑，保存后刷新章节摘要、检索片段与实体状态。
...
- **Agent Maestro 接入**：开发环境可在无 `backend/.env` 时 fallback 读取 `.claude/settings.json` 的 `ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_MODEL`。
...
| **伏笔/支线追踪** | 结构化 Foreshadow 表、上下文注入、章节状态建议、跨章仪表盘已完成；待补自动停滞检测与状态批量建议 | 阶段 7 🚧 |
...
| 日期 | 能力 | 说明 |
| --- | --- | --- |
| 2026-06-03 | Agent Maestro 接入 | 后端支持从 `.claude/settings.json` 自动读取本地 Anthropic 兼容代理配置，已通过 tool-use 与 API E2E 烟测。 |
...
1. 长篇一致性：为伏笔/支线追踪补自动停滞检测与状态批量建议。
